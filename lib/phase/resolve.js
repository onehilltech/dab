  ...
  resolve (value) {
    function resolved (resolver, original) {
      return function (result) {
        ...
        if (!isEmpty (resolver.unresolved))
    ...
    if (isFunction (value)) {
      ...
      return Promise.resolve (result).then (this.resolve.bind (this));
    }
      ...
      const mapped = value.map ((item, index) => {
        ...
        return resolver.resolve (item).then (resolved (resolver, item).bind (this));
      });
      ...
      const mapping = mapValues (value, (value, key) => {
        ...
        return resolver.resolve (value).then (resolved (resolver, value).bind (this));
      });
 ...
 */
function resolve (value, data, opts = {}) {
