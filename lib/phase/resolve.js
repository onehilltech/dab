...
Resolver.prototype.resolve = function (value, callback) {
  function resolved (path, value, callback) {
    return function (err, result) {
      if (err)
        return callback (err);

      if (result === undefined)
        this._unresolved[path] = value;
  ...
  if (_.isFunction (value)) {
    ...
    process.nextTick (function () {
      ...
      async.map (wrapped, function (item, callback) {
        ...
        resolver.resolve (item.value, resolved.call (this, resolver.path, value, callback));
      }.bind (this), callback);
    }.bind (this));
  }
    ...
    process.nextTick (function () {
      async.mapValues (value, function (value, key, callback) {
        ...
        resolver.resolve (value, resolved.call (this, resolver.path, value, callback));
      }.bind (this), callback);
    }.bind (this));
  }
